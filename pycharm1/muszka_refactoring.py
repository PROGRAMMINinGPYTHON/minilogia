...
def side(ile, jak_wys, kolumny):
    for i in range(ile):
        # if kolumny == kolumny_left:
        #     kolumny_left(ile, jak_wys + (2 * i))
        # else:
        #     kolumny_right(ile, jak_wys +(2*i))
        kolumny(ile,jak_wys+(2*i))
        rt(90)
        fd(a * math.sqrt(2))
        lt(90)
...
def muszka(ile):
    ...
    side(ile, 2,kolumny_left)
    pu()
    setpos((a / 2) * math.sqrt(2), -a * math.sqrt(2))
    setheading(0)
    fd(2 * a)
    side(ile, 2, kolumny_right)
    pu()
    setpos(0, 0)
    setheading(-45)
    fd(a)
